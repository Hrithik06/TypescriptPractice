...
const hrithik: User = {dbId:1545, email:"h@hrithik.com", userId:6565, githubToken:"sdsf",
    startTrial:()=>{
        return "start trial"
    },
    getCoupon: (name: "trial10", off:10)=>{ // see argument name is different from interface value->off couponname->name allowed
        return 10
    }
}
...
const hrithik2: Admin = {dbId:1545, email:"h@hrithik.com", userId:6565, role: "admin", githubToken:"sdsf",
    startTrial:()=>{
        return "start trial"
    },
    getCoupon: (name: "trial10", off:10)=>{ // see argument name is different from interface value->off couponname->name allowed
